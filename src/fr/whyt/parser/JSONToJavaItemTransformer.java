...
import fr.whyt.craft.enums.CraftingDiscipline;
import fr.whyt.craft.enums.CraftingFlag;
import fr.whyt.item.Item;
import fr.whyt.item.components.Attribute;
import fr.whyt.item.components.Buff;
...
import fr.whyt.item.extend.Consumable;
import fr.whyt.item.extend.Container;
import fr.whyt.item.extend.Gathering;
import fr.whyt.item.extend.Gizmo;
import fr.whyt.item.extend.Tool;
import fr.whyt.item.extend.Trinket;
import fr.whyt.item.extend.UpgradeComponent;
import fr.whyt.item.extend.Weapon;
import fr.whyt.persona.enums.Restriction;
import fr.whyt.settings.SupportedLanguage;

public class JSONToJavaItemTransformer {
	private JSONToJavaItemTransformerUtils utils = new JSONToJavaItemTransformerUtils();
	...
	private InfixUpgrade parseInfixUpgrade(JSONObject obj) {
		...
		if (obj.get("infix_upgrade") != null) {
			...
			for (Object attributeO : attributesObj) {
				...
				attributes
						.add(new Attribute(
								AttributeType.resolve((String) attributeObj
										.get("attribute")), Integer
										.parseInt((String) attributeObj
												.get("modifier"))));
			}
			...
			if (infixObj.get("buff") != null) {
				...
				buff = new Buff((String) buffObj.get("skill_id"),
						(String) buffObj.get("description"));
			}
	...
	private List<InfusionSlot> parseInfusionSlots(JSONObject obj) {
		...
		List<InfusionSlot> infusionSlots = new ArrayList<InfusionSlot>(
				infusionSlotsObj.size());

		for (Object infusionSlotO : infusionSlotsObj) {
			JSONObject infusionSlotObj = (JSONObject) infusionSlotO;

			JSONArray infusionFlagsObj = (JSONArray) infusionSlotObj
					.get("flags");
			List<InfusionSlotFlag> infusionFlags = new ArrayList<InfusionSlotFlag>(
					infusionFlagsObj.size());
			...
			infusionSlots.add(new InfusionSlot(infusionFlags, utils
					.parseGracefullyLong(infusionSlotObj, "item_id")));
	...
	public Item transformItem(JSONObject obj) {
		...
		if (ItemClass.WEAPON.equals(itemClass)) {
			...
			weapon = new Weapon(
					ItemType.resolve((String) weaponObj.get("type")),
					DamageType.resolve((String) weaponObj.get("damage_type")),
					Long.parseLong((String) weaponObj.get("min_power")),
					Long.parseLong((String) weaponObj.get("max_power")),
					Long.parseLong((String) weaponObj.get("defense")),
					infusionSlots, infixUpgrade);
		}
		...
		if (ItemClass.TRINKET.equals(itemClass)) {
			...
			trinket = new Trinket(ItemType.resolve((String) trinketObj
					.get("type")), infusionSlots, infixUpgrade,
					utils.parseGracefullyLong(trinketObj, "suffix_item_id"));
		}
		...
		if (ItemClass.BAG.equals(itemClass)) {
			...
			bag = new Bag(Long.parseLong((String) bagObj.get("size")),
					bagModifiers);
		}
		...
		if (ItemClass.CONTAINER.equals(itemClass)) {
			...
			container = new Container(
					ContainerType.resolve((String) containerObj.get("type")));
		}
		...
		if (ItemClass.CONSUMABLE.equals(itemClass)) {
			...
			ConsumableType consumableType = ConsumableType
					.resolve((String) consumableObj.get("type"));
			UnlockType unlockType = null;
			Long colorId = null;
			Long recipeId = null;
			if (ConsumableType.UNLOCK.equals(consumableType)) {
				unlockType = UnlockType.resolve((String) consumableObj
						.get("unlock_type"));
				switch (unlockType) {
				case CRAFTING_RECIPE:
					recipeId = Long.parseLong((String) consumableObj
							.get("recipe_id"));
					break;
				case DYE:
					colorId = Long.parseLong((String) consumableObj
							.get("color_id"));
					break;
				default:
					break;
			...
			consumable = new Consumable(consumableType,
					(String) consumableObj.get("description"),
					utils.parseGracefullyLong(consumableObj, "duration_ms"),
					unlockType, colorId, recipeId);
		}

		UpgradeComponent upgradeComponent = null;
		if (ItemClass.UPGRADE_COMPONENT.equals(itemClass)) {
			JSONObject upgradeComponentObj = (JSONObject) obj
					.get("upgrade_component");
			...
			JSONArray upgradeComponentFlagsObj = (JSONArray) upgradeComponentObj
					.get("flags");

			for (Object flagO : upgradeComponentFlagsObj) {
				upgradeComponentFlags.add(UpgradeComponentFlag
						.resolve(((String) flagO)));
			}

			JSONArray infusionSlotFlagsObj = (JSONArray) upgradeComponentObj
					.get("infusion_upgrade_flags");
			List<InfusionSlotFlag> infusionSlotFlags = new ArrayList<InfusionSlotFlag>(
					infusionSlotFlagsObj.size());
			for (Object infusionSlotFlagO : infusionSlotFlagsObj) {
				infusionSlotFlags.add(InfusionSlotFlag
						.resolve(((String) infusionSlotFlagO)));
			}

			List<String> bonuses = null;
			if (upgradeComponentObj.get("bonuses") != null) {
				JSONArray bonusesObj = (JSONArray) upgradeComponentObj
						.get("bonuses");
			...
			upgradeComponent = new UpgradeComponent(
					UpgradeComponentType.resolve((String) upgradeComponentObj
							.get("type")), upgradeComponentFlags, infixUpgrade,
					(String) upgradeComponentObj.get("suffix"),
					infusionSlotFlags, bonuses);
		}
		...
		if (ItemClass.BACK.equals(itemClass)) {
			...
			back = new Back(infusionSlots, infixUpgrade,
					(String) backObj.get("suffix_item_id"));
		}
		...
		if (ItemClass.GATHERING.equals(itemClass)) {
			...
			gathering = new Gathering(
					GatheringType.resolve((String) gatheringObj.get("type")));
		}
		...
		if (ItemClass.TOOL.equals(itemClass)) {
			...
			tool = new Tool(ToolType.resolve((String) toolObj.get("type")),
					Long.parseLong((String) toolObj.get("charges")));
		}

		return new Item(Long.parseLong((String) obj.get("item_id")),
				(String) obj.get("name"), (String) obj.get("description"),
				Long.parseLong((String) obj.get("level")),
				Rarity.resolve((String) obj.get("rarity")),
				Long.parseLong((String) obj.get("vendor_value")), gameTypes,
				flags, restrictions, utils.parseGracefullyLong(obj,
						"suffix_item_id"), itemClass, armor, weapon, bag,
				container, consumable, trinket, upgradeComponent, back,
	...
	public Recipe transformRecipe(JSONObject obj) {
		...
		for (Object ingredientO : ingredientsObj) {
			...
			ingredients.add(new Ingredient(Long
					.parseLong((String) ingredientObj.get("count")), (Long
					.parseLong((String) ingredientObj.get("item_id")))));
		}
		...
		List<CraftingDiscipline> craftingDisciplines = new ArrayList<CraftingDiscipline>(
				disciplinesObj.size());
		for (Object disciplineO : disciplinesObj) {
			craftingDisciplines.add(CraftingDiscipline
					.resolve((String) disciplineO));
		}

		return new Recipe(Long.parseLong((String) obj.get("recipe_id")),
				ingredients, Long.parseLong((String) obj.get("min_rating")),
				Long.parseLong((String) obj.get("time_to_craft_ms")),
				Long.parseLong((String) obj.get("output_item_id")),
				Long.parseLong((String) obj.get("output_item_count")),
				ItemType.resolve((String) obj.get("type")),
				craftingDisciplines, flags);
	}
	...
	public WvWMatchDetail transformWvWMatchDetails(JSONObject obj) {
		...
		for (Object mapO : mapsObj) {
			...
			List<WvWObjective> objectives = new ArrayList<WvWObjective>(
					objectivesObj.size());

			for (Object objectiveO : objectivesObj) {
				JSONObject objectiveObj = (JSONObject) objectiveO;

				objectives.add(new WvWObjective((Long) objectiveObj.get("id"),
						(String) objectiveObj.get("owner_guild"), WvWSide
								.resolve((String) objectiveObj.get("owner"))));
			}

			JSONArray scores = (JSONArray) mapObj.get("scores");
			wvwMaps.add(new WvWMap(new Scores((Long) scores.get(0),
					(Long) scores.get(1), (Long) scores.get(2)), WvWMapType
					.resolve((String) mapObj.get("type")), objectives));
		}

		JSONArray scores = (JSONArray) obj.get("scores");
		return new WvWMatchDetail(new Scores((Long) scores.get(0),
				(Long) scores.get(1), (Long) scores.get(2)),
				(String) obj.get("match_id"), wvwMaps);
	}

	public Color transformColor(JSONObject colorsObj) {
		ColorHue defaultColor = transformColorHue((JSONObject) colorsObj
				.get("default"));
		ColorHue metalColor = transformColorHue((JSONObject) colorsObj
				.get("metal"));
		ColorHue clothColor = transformColorHue((JSONObject) colorsObj
				.get("cloth"));
		ColorHue leatherColor = transformColorHue((JSONObject) colorsObj
				.get("leather"));
		RGB rgb = transformRGB((JSONArray) colorsObj.get("base_rgb"));
		return new Color((String) colorsObj.get("name"), defaultColor,
				metalColor, clothColor, leatherColor, rgb);
	}
	...
	private ColorHue transformColorHue(JSONObject colorHueObj) {
		if (colorHueObj != null) {
			...
			return new ColorHue(
					utils.parseDouble(colorHueObj.get("brightness")),
					utils.parseDouble(colorHueObj.get("contrast")),
					utils.parseDouble(colorHueObj.get("hue")),
					utils.parseDouble(colorHueObj.get("saturation")),
					utils.parseDouble(colorHueObj.get("lightness")), rgb);
		} else {
			return null;
		}
	}
	...
	private RGB transformRGB(JSONArray rgbObj) {
		return new RGB((Long) rgbObj.get(0), (Long) rgbObj.get(1),
				(Long) rgbObj.get(2));
	}
	...
	public Guild transformGuildDetail(JSONObject obj) {
		...
		if (emblemObj != null) {
			...
			List<GuildEmblemFlag> flags = new ArrayList<GuildEmblemFlag>(
					flagsObj.size());
			for (Object flagO : flagsObj) {
				flags.add(GuildEmblemFlag.resolve((String) flagO));
			}

			emblem = new GuildEmblem((Long) emblemObj.get("background_id"),
					(Long) emblemObj.get("foreground_id"), flags,
					(Long) emblemObj.get("foreground_primary_color_id"),
					(Long) emblemObj.get("foreground_secondary_color_id"),
					(Long) emblemObj.get("background_color_id"));
		}

		return new Guild((String) obj.get("guild_id"), (String) obj.get("tag"),
				(String) obj.get("guild_name"), emblem);
	}
	...
	public EventDetail transformEventDetail(JSONObject eventObj) {
		...
		LocationType type = LocationType.resolve((String) locationObj
				.get("type"));
		...
		if (LocationType.POLY.equals(type)) {
			...
			zRange = new ZRange(utils.parseDouble(zRangeObj.get(0)),
					utils.parseDouble(zRangeObj.get(1)));

		}

		EventLocation location = new EventLocation(
				utils.parsePoint3D((JSONArray) locationObj.get("center")),
				utils.parseDouble(locationObj.get("rotation")),
				utils.parseDouble(locationObj.get("radius")),
				utils.parseDouble(locationObj.get("height")), zRange, points,
				type);

		EventDetail eventDetail = new EventDetail(flags,
				(Long) eventObj.get("map_id"), (Long) eventObj.get("level"),
				location, (String) eventObj.get("name"));

		return eventDetail;
	}

	public Continent transformContinent(JSONObject continentObj) {
		List<Long> floors = utils.parseLongList((JSONArray) continentObj
				.get("floors"));

		return new Continent((String) continentObj.get("name"),
				utils.parsePoint2D((JSONArray) continentObj
						.get("continent_dims")),
				(Long) continentObj.get("min_zoom"),
				(Long) continentObj.get("max_zoom"), floors);
	}

	public AreaMap transformAreaMap(JSONObject mapObj) {
		System.out.println(mapObj);

		List<Long> floors = utils.parseLongList((JSONArray) mapObj
				.get("floors"));
		MapRectangle continentRectangle = utils
				.parseRectangle((JSONArray) mapObj.get("continent_rect"));
		MapRectangle mapRectangle = utils.parseRectangle((JSONArray) mapObj
				.get("map_rect"));
		return new AreaMap((String) mapObj.get("continent_name"),
				(Long) mapObj.get("continent_id"),
				(String) mapObj.get("region_name"),
				(Long) mapObj.get("region_id"),
				(String) mapObj.get("map_name"),
				(Long) mapObj.get("min_level"), (Long) mapObj.get("max_level"),
				mapRectangle, continentRectangle, floors,
				(Long) mapObj.get("default_floor"));
	}
	...
	public MapFloor transformMapFloor(JSONObject mapFloorObj) {
		...
		for (String regionKey : regionKeys) {
			...
			for (String mapKey : mapKeys) {
				...
				MapRectangle continentRectangle = utils
						.parseRectangle((JSONArray) mapObj
								.get("continent_rect"));
				MapRectangle mapRectangle = utils
						.parseRectangle((JSONArray) mapObj.get("map_rect"));

				JSONArray skillChallengesObj = (JSONArray) mapObj
						.get("skill_challenges");
				List<SkillChallenge> skillChallenges = new ArrayList<SkillChallenge>(
						skillChallengesObj.size());
				for (Object skillChallengesO : skillChallengesObj) {
					skillChallenges
							.add(new SkillChallenge(
									utils.parsePoint2D((JSONArray) ((JSONObject) skillChallengesO)
											.get("coord"))));
				}

				JSONArray poisObj = (JSONArray) mapObj
						.get("points_of_interest");
				List<PointOfInterest> pois = new ArrayList<PointOfInterest>(
						poisObj.size());
				for (Object poiO : poisObj) {
					JSONObject poiObj = (JSONObject) poiO;
					pois.add(new PointOfInterest(utils
							.parsePoint2D((JSONArray) poiObj.get("coord")),
							(String) poiObj.get("name"), (Long) poiObj
									.get("floor"), (Long) poiObj.get("poi_id"),
							PoIType.resolve((String) poiObj.get("type"))));
				}
				...
				for (Object tasksO : tasksObj) {
					...
					tasks.add(new Heart(utils.parsePoint2D((JSONArray) taskObj
							.get("coord")), (Long) taskObj.get("level"),
							(String) taskObj.get("objective"), (Long) taskObj
									.get("task_id")));
				}

				JSONArray sectorsObj = (JSONArray) mapObj.get("sectors");
				List<MapAreaSector> sectors = new ArrayList<MapAreaSector>(
						sectorsObj.size());
				for (Object sectorO : sectorsObj) {
					JSONObject sectorObj = (JSONObject) sectorO;
					sectors.add(new MapAreaSector(utils
							.parsePoint2D((JSONArray) sectorObj.get("coord")),
							(Long) sectorObj.get("level"), (String) sectorObj
									.get("name"), (Long) sectorObj
									.get("sector_id")));
				}

				maps.put(
						Long.parseLong(mapKey),
						new MapAreaFloor((String) mapObj.get("name"),
								(Long) mapObj.get("min_level"), (Long) mapObj
										.get("max_level"), mapRectangle,
								continentRectangle, (Long) mapObj
										.get("default_floor"), skillChallenges,
								pois, tasks, sectors));
			}

			regions.put(
					Long.parseLong(regionKey),
					new MapRegion((String) regionObj.get("name"), utils
							.parsePoint2D((JSONArray) regionObj
									.get("label_coord")), maps));
		}

		return new MapFloor(utils.parsePoint2D((JSONArray) mapFloorObj
				.get("texture_dims")), regions);
	}
}
